LIB_PHspline <- function(formula, data, k){
  ...
  .flex<-flexsurvspline(formula, data = data, scale="hazard", k=k,
                        hessian=F, method="Nelder-Mead") # If "hazard", the log cumulative hazard is modelled as a spline function.
